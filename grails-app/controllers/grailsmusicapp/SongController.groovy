            ...
            if (!songExists) {
                try {
                   // createSongService.addSong(params.songname, params.songalbum, params.songgenre, params.songartist)
                    def artistid=Artist.findByArtistname(params.songartist)
                    println(artistid)
                    def s= new Song(songname: params.songname,album: params.songalbum,genre: params.songgenre, artist: artistid,isdeleted: false)
                    s.save(flush: true)
