...
class ViewSongService {
    ...
    def getDetails(Integer id) {
        Sql sql= new Sql(dataSource)
        def details = sql.rows("""select song.idsong,song.songname,song.album,song.genre,artist.artistname from song INNER JOIN artist ON song.artist_id=artist.idartist where song.idsong=$id""")
        return details
    }
